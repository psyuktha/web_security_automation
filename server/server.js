...
app.post("/api/scan/start", authenticateToken, async (req, res) => {
  try {
    ...
    // Pre-insert running status so frontend can poll
    const db = getDB();
    const collection = db.collection("scan_history");
    await collection.insertOne({
      scanId: scanId, // Store specific custom ID field
    ...
    // Run scan in background and update results
    scanPromise
      .then(async (result) => {
        if (result.success) {
          ...
          await collection.updateOne({ scanId: scanId }, { $set: { status: "failed", error: result.error }});
        }
      })
      .catch((error) => {
        console.error(`❌ Scan error: ${error.message}`);
        collection.updateOne({ scanId: scanId }, { $set: { status: "failed", error: error.message }});
      });
...
app.get("/api/scan/status/:scanId", authenticateToken, async (req, res) => {
  try {
    ...
    // Find scan by ID pattern (scanId custom field)
    ...
    res.json({
      ...
      progress: scan.status === "running" ? 50 : 100,
