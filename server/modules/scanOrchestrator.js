...
export const runSecurityScan = async (targetUrl, attackTypes) => {
  ...
  try {
    ...
    if (attackTypes.sqlInjection) {
      ...
      // ── FIX 3: Pass endpointsWithParams, not all endpoints ─────
      ...
      vulnerabilities = attackResults.map((result) => {
        ...
        const baseEvidence = result.evidence || "SQL injection pattern detected in response";
        const detailedEvidence = `${baseEvidence}\nPayload: ${result.payload}\nStatus code: ${result.statusCode}\nResponse snippet: ${responseSnippet}`;

        return {
          type: "SQL Injection",
          injectionType: result.type || "unknown",       // error-based, union-based, etc.
          confidence: result.confidence || "low",         // high / medium / low
          severity: determineSeverity(result),
          endpoint: result.endpoint,
          method: result.method,
          payload: result.payload,
          description: `SQL injection vulnerability detected in ${result.endpoint} — ${result.type || "unknown type"}`,
          evidence: detailedEvidence,
          responseTime: result.responseTime,
          statusCode: result.statusCode,
          responseBody: result.responseBody,
